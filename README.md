...
- **Privileged-looking public functions with no capability param** — a `public` function named like `mint_`, `admin_`, `upgrade_`, `freeze_`, `pause_`/`unpause_`, `init_admin`, or `transfer_ownership` that takes no capability-style parameter (`Cap`, `Admin`, `Owner`, `Witness`, `Publisher`, `Ticket`, `Receipt`, `Proof`, `Permit`) gets flagged — unless the function name itself contains "cap" (e.g. `mint_account_cap`), since minting/burning a capability *of itself* is a normal self-registration pattern (there's no prior capability to require — that's the point of the function), not a missing access check. The verb list is intentionally narrow and high-signal — generic Move primitives like `add_`/`remove_`/`set_`/`update_` were deliberately left out after testing against Sui's own framework package (`0x2`) surfaced them as noise, not real signal.
